...
const (
	...
	// inventory to a peer.
	trickleTimeout = 100 * time.Microsecond
)
...
type Config struct {
	...
	// NodePrivateKey is used to create and attach a sign that verify authentication
	// of node itself. Other nodes will connect with this by checking an existance of
	// address in permission index.
	NodePrivateKey *btcec.PrivateKey

	// DB is used to check permission index
	DB database.DB

	// Manager to check node permission
	PermissionManager permission.Manager

	StallResponseTimeout time.Duration
}
...
func (p *Peer) pushVersionMsg() error {
	...
	// add node sign to verify identity
	if p.cfg.NodePrivateKey != nil {
		// generate sign
		sign, err := generatePeerSignature(p.cfg.NodePrivateKey, msg.AddrMe)
		if nil != err {
			return err
		}

		// attach the generated sign to msg
		msg.AddNodeSign(sign)
	}
...
func (p *Peer) handleVersionMsg(msg *wire.MsgVersion) error {
	...
	// check a node permission
	if p.cfg.PermissionManager != nil {
		err := p.cfg.DB.View(func(dbTx database.Tx) error {
			var err error
			err = checkPeerPermission(p.cfg.PermissionManager, dbTx, msg, p.cfg.ChainParams)
			return err
		})
		if nil != err {
			p.PushRejectMsg(msg.Command(), wire.RejectInvalid,
				err.Error(), nil, true)
...
func (p *Peer) readMessage() (wire.Message, []byte, error) {
	...
	case *wire.MsgReject:
		log.Warnf("%v", newLogClosure(func() string {
			// Warn summary of reject message.
...
func (p *Peer) maybeAddDeadline(pendingResponses map[string]time.Time, msgCmd string) {
	...
	stallResponseTimeout := p.cfg.StallResponseTimeout
	log.Tracef("StallResponseTimeout set to %d", p.cfg.StallResponseTimeout/time.Second)
...
func (p *Peer) queueHandler() {
	...
	pending := []*list.List{
		BlockQ:   list.New(), // The block pending list is the first.
		DefaultQ: list.New(),
	}

	getQueue := func(msg *outMsg) *list.List {
		blockQ, defaultQ := pending[BlockQ], pending[DefaultQ]

		switch m := msg.msg.(type) {
		case *wire.MsgBlock:
			log.Tracef("BlockQ selected for %v", m.Command())
			return blockQ
		case *wire.MsgGetData:
			if len(m.InvList) > 0 && m.InvList[0].Type == wire.InvTypeBlock {
				log.Tracef("BlockQ selected for %v", m.Command())
				return blockQ
			}
		case *wire.MsgInv:
		...
		log.Tracef("DefaultQ selected for %v", msg.msg.Command())
		return defaultQ
	}
	...
	queuePacket := func(msg outMsg, waiting bool) bool {
		if !waiting {
			...
			getQueue(&msg).PushBack(msg)
		}
		// we are always waiting now.
		return true
	}

	getNext := func() interface{} {
		// Send block message with top priority: high load (high transactions
		// per sec) may lead to a large delay in block synchronization because
		// all the p2p channels are flooded by transaction messages.
		for _, q := range pending {
			next := q.Front()
			if next != nil {
				return q.Remove(next)
			}
		}
...
out:
	...
	drainPendingQueue := func(q *list.List) {
...
//
// This function is safe for concurrent access.
func (p *Peer) QueueInventory(invVect *wire.InvVect) bool {
	// Don't add the inventory to the send queue if the peer is already
	// known to have it.
	if p.knownInventory.Exists(invVect) {
		log.Debugf("Reject to relay a known inv (type: %v, hash: %v, dest: %v)",
			invVect.Type, invVect.Hash, p.Addr())
		return false
	}
