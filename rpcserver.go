...
const (
	...
	// padding format.
	hash1Len = (1 + ((wire.HashSize + 8) / fastsha256.BlockSize)) * // nolint: varcheck
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		return nil, internalRPCError(err.Error(), errDeserializeBlock)
	}

	// Get the block height from chain.
	blockHeight, err := s.chain.BlockHeightByHash(hash)
	if err != nil {
		return nil, internalRPCError(err.Error(), errNotExistBlockHeight)
	}
	...
	if blockHeight < best.Height {
		...
		if err != nil {
			return nil, internalRPCError(err.Error(), errNoNextBlock)
		}
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		return nil, internalRPCError(err.Error(), errDeserializeBlockHeader)
	}
...
func encodeTemplateID(prevHash *wire.ShaHash, lastGenerated time.Time) string {
	return fmt.Sprintf("%s-%d", prevHash, lastGenerated.Unix())
}
...
func (state *gbtWorkState) updateBlockTemplate(s *rpcServer, useCoinbaseValue bool) error {
	...
	if template == nil || state.prevHash == nil ||
		...
		UpdateBlockTime(msgBlock, s.server.blockManager) // nolint: errcheck
...
func handleGetBlockTemplateLongPoll(s *rpcServer, longPollID string, useCoinbaseValue bool, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !prevHash.IsEqual(prevTemplateHash) ||
		...
		result, e := state.blockTemplateResult(useCoinbaseValue,
			&submitOld)
		if e != nil {
			state.Unlock()
			return nil, e
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		...
		if _, ok := err.(RuleError); ok {
			rpcsLog.Warnf("Rejected transaction %v: %v", tx.Sha(),
				err)
		} else {
	...
	s.server.AnnounceNewTransactionsImmediately(acceptedTxs)
...
func capBlockSet(s *rpcServer, block *btcutil.Block) {
	s.capBlock = block
	rpcsLog.Debugf("CAP block %s set", block.Sha())
}

func capBlockInit(s *rpcServer) {
	if s.capBlock != nil {
		rpcsLog.Debugf("CAP block init (org: %s)", s.capBlock.Sha())
		s.capBlock = nil
	}
}

func processDecodedBlock(s *rpcServer, block *btcutil.Block) (interface{}, error) {
	defer capBlockInit(s)
...
func getBlockFromCmd(s *rpcServer, cmd interface{}) (*btcutil.Block, error) {
...
func needFastReply() bool {
	return len(cfg.CAPEndpoint) > 0 && cfg.CAPFastReply == true
}

// handleSubmitBlock implements the submitblock command.
func handleSubmitBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	block, err := getBlockFromCmd(s, cmd)
	if err != nil {
		return nil, err
	}
	...
	return processDecodedBlock(s, block)
}
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	wire.WriteVarString(&buf, 0, c.Message)                   // nolint: errcheck
...
// handleCheckPermission implements the checkpermission command.
func handleCheckPermission(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*btcjson.CheckPermissionCmd)
	hasPermission := false

	permissions := strings.Split(c.Permission, "|")
	var permissionMarker byte = 0x0

	for _, permissionStr := range permissions {
		switch permissionStr {
		case permission.AdminPermission:
			permissionMarker |= permission.AdminMarker
		case permission.WriterPermission:
			permissionMarker |= permission.WriterMarker
		case permission.MinerPermission:
			permissionMarker |= permission.MinerMarker
		case permission.NodePermission:
			permissionMarker |= permission.NodeMarker
		default:
			return false, fmt.Errorf("Unsupported type of permission: %s. Permission must be one of %s, %s, %s, %s ",
				permissionStr, permission.AdminPermission, permission.WriterPermission,
				permission.MinerPermission, permission.NodePermission)
		}
	}

	err := s.server.db.View(func(dbTx database.Tx) error {
		hasPermission = s.server.permIndex.CheckPermission(dbTx, c.Address, permissionMarker)
		return nil
	})
	if nil != err {
		context := "Failed to check permission"
		return false, internalRPCError(err.Error(), context)
	}
...
func (s *rpcServer) Stop() error {
	...
	AdmWarn(rpcsLog, "RPC server shutting down")
	...
	close(s.quit)
	s.wg.Wait()
	AdmInfo(rpcsLog, "RPC server shutdown complete")
	return nil
}
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	// For the fast reply mode, process (conntect the request block
	// here. Ignore the return values since error basically cannot occur.
	if needFastReply() && s.capBlock != nil {
		rpcsLog.Debugf("CAP block - %s", s.capBlock.Sha())
		processDecodedBlock(s, s.capBlock) // nolint: errcheck
	}
}
...
func (s *rpcServer) Start() {
	...
	for _, listener := range s.listeners {
		...
		go func(listener net.Listener) {
			...
			httpServer.Serve(listener) // nolint: errcheck
