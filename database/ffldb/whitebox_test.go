...
func resetDatabase(tc *testContext) bool {
	...
	err := tc.db.Update(func(tx database.Tx) error {
		...
		_, err = tx.Metadata().CreateBucket(txIndexKey)
		return err
	})
	if err != nil {
...
func testBlockFileErrors(tc *testContext) bool {
	...
	tx0Hash := tc.blocks[0].Transactions()[0].Sha()
	testName = "readBlock invalid file number"
	invalidLoc := blockLocation{
		blockFileNum: ^uint32(0),
		blockLen:     80,
	}
	_, err = store.readBlock(invalidLoc)
	if !checkDbError(tc.t, testName, err, database.ErrDriverSpecific) {
		return false
	}
	testName = "readBlockRegion invalid file number"
	invalidTxLoc := transactionLocation{
		blockFileNum: ^uint32(0),
		txOffset:     0,
		txLen:        80,
	}
	_, _, err = store.readBlockLocation(&invalidTxLoc)
	if !checkDbError(tc.t, testName, err, database.ErrDriverSpecific) {
		return false
	}
	...
	err = tc.db.View(func(tx database.Tx) error {
		...
		_, _, e = tx.FetchTransaction(tx0Hash)
		if !checkDbError(tc.t, testName, e, wantErrCode) {
			return errSubTestFail
		}

		testName = "FetchBlockRegions closed file"
		_, _, e = tx.FetchTransactions([]wire.ShaHash{*tx0Hash})
		if !checkDbError(tc.t, testName, e, wantErrCode) {
			return errSubTestFail
		}
...
func testCorruption(tc *testContext) bool {
	...
	block0Bytes, _ := tc.blocks[0].Bytes()
	block0Hash := tc.blocks[0].Sha()
	updateSerializedChecksumEnabled = true
	tests := []struct {
		offset      uint32
		fixChecksum bool
		wantErrCode database.ErrorCode
