...
import (
	...
	ldberrors "github.com/coinstack/goleveldb/leveldb/errors"
	...
	"github.com/coinstack/coinstackd/database/internal/treap"
...
var (
	...
	errNotExistTxLoc = errors.New("failed to retrieve transaction location")
)
...
func (s bulkFetchDataSorter) Less(i, j int) bool {
	...
	return s[i].blockOffset < s[j].blockOffset
}
...
func (b *bucket) RangeCursor(prefix []byte) database.Cursor {
	// Ensure transaction state is valid.
	if err := b.tx.checkClosed(); err != nil {
		return &cursor{bucket: b}
	}
	...
	// Create the cursor and setup a runtime finalizer to ensure the
	// iterators are released when the cursor is garbage collected.
	key := bucketizedKey(b.id, prefix)
	c := newCursor(b, key, ctKeys)
	runtime.SetFinalizer(c, cursorFinalizer)
	return c
}
...
type TreapBucket struct {
	keys     *treap.Mutable
	iterLock sync.RWMutex
	iters    []*treap.Iterator
}

func NewTreapBucket() database.Bucket {
	return &TreapBucket{keys: treap.NewMutable()}
}

var errNotSupported = errors.New("not supported API")

func (b *TreapBucket) Bucket(key []byte) database.Bucket {
	panic(errNotSupported)
}

func (b *TreapBucket) CreateBucket(key []byte) (database.Bucket, error) {
	panic(errNotSupported)
}

func (b *TreapBucket) CreateBucketIfNotExists(key []byte) (database.Bucket, error) {
	panic(errNotSupported)
}

func (b *TreapBucket) DeleteBucket(key []byte) error {
	return errNotSupported
}

func (b *TreapBucket) ForEach(func(k, v []byte) error) error {
	return errNotSupported
}

func (b *TreapBucket) ForEachBucket(func(k []byte) error) error {
	return errNotSupported
}

func (b *TreapBucket) Cursor() database.Cursor {
	c := newTempCursor(b.keys, nil, b)
	runtime.SetFinalizer(c, tempCursorFinalizer)
	return c
}

func (b *TreapBucket) RangeCursor(prefix []byte) database.Cursor {
	c := newTempCursor(b.keys, prefix, b)
	runtime.SetFinalizer(c, tempCursorFinalizer)
	return c
}

func (b *TreapBucket) Writable() bool {
	return true
}

func (b *TreapBucket) Put(key, value []byte) error {
	b.keys.Put(key, value)
	b.notifyIters()
	return nil
}

func (b *TreapBucket) Get(key []byte) []byte {
	return b.keys.Get(key)
}

func (b *TreapBucket) Delete(key []byte) error {
	b.keys.Delete(key)
	b.notifyIters()
	return nil
}

func (b *TreapBucket) addIter(iter *treap.Iterator) {
	b.iterLock.Lock()
	b.iters = append(b.iters, iter)
	b.iterLock.Unlock()
}

func (b *TreapBucket) notifyIters() {
	b.iterLock.RLock()
	for _, cursor := range b.iters {
		cursor.ForceReseek()
	}
	b.iterLock.RUnlock()
}

func (b *TreapBucket) removeIter(iter *treap.Iterator) {
	b.iterLock.Lock()
	for i := 0; i < len(b.iters); i++ {
		if b.iters[i] == iter {
			copy(b.iters[i:], b.iters[i+1:])
			b.iters[len(b.iters)-1] = nil
			b.iters = b.iters[:len(b.iters)-1]
		}
	}
	b.iterLock.Unlock()
}
...
func newTempCursor(keys *treap.Mutable, prefix []byte, bucket *TreapBucket) database.Cursor {
	keyRange := util.BytesPrefix(prefix)
	keyIter := keys.Iterator(keyRange.Start, keyRange.Limit)
	bucket.addIter(keyIter)
	return &tempCursor{keyIter, bucket, false}
}

func (c *tempCursor) Bucket() database.Bucket {
	panic(errNotSupported)
}

func (c *tempCursor) Delete() error {
	panic(errNotSupported)
}

func (c *tempCursor) First() bool {
	return c.iter.First()
}

func (c *tempCursor) Last() bool {
	panic(errNotSupported)
}

func (c *tempCursor) Next() bool {
	return c.iter.Next()
}

func (c *tempCursor) Prev() bool {
	panic(errNotSupported)
}

func (c *tempCursor) Seek(seek []byte) bool {
	panic(errNotSupported)
}

func (c *tempCursor) Key() []byte {
	return c.iter.Key()
}

func (c *tempCursor) Value() []byte {
	return c.iter.Value()
}

func (c *tempCursor) ForceReseek() {
	c.iter.ForceReseek()
}

func (c *tempCursor) Release() {
	if !c.released {
		c.bucket.removeIter(c.iter)
		c.released = true
	}
}

func tempCursorFinalizer(cursor *tempCursor) {
	cursor.Release()
}
...
type transaction struct {
	...
	// transaction that need to be stored on commit. The pendingTransactions map is
	// kept to allow trace a block hash by transaction id
	pendingTransactions map[wire.ShaHash]int

	pendingDisconnectBlock []*btcutil.Block
...
func (tx *transaction) StoreBlock(block *btcutil.Block) error {
	...
	// Add Info to Map: Transaction to BlockData
	if tx.pendingTransactions == nil {
		tx.pendingTransactions = make(map[wire.ShaHash]int)
	}
	for _, aTx := range block.Transactions() {
		tx.pendingTransactions[*aTx.Sha()] = len(tx.pendingBlockData)
	}
...
func (tx *transaction) DisconnectBlock(block *btcutil.Block) error {
	// Ensure transaction state is valid.
	if err := tx.checkClosed(); err != nil {
		return err
	}
	...
	// Ensure the transaction is writable.
	if !tx.writable {
		str := "disconnect block requires a writable database transaction"
		return makeDbErr(database.ErrTxNotWritable, str, nil)
	}

	// Add the block to be disconnected to the list of disconnecting blocks
	// when the transaction is committed.
	tx.pendingDisconnectBlock = append(tx.pendingDisconnectBlock, block)

	log.Tracef("Added block %s to deleting blocks", block.Sha())
...
func (tx *transaction) FetchBlock(hash *wire.ShaHash) ([]byte, error) {
	...
	blockBytes, err := tx.db.store.readBlock(location)
	if err != nil {
		return nil, err
	}
...
// fetchPendingTxRegion attempts to fetch the provided region from any block which
...
func (tx *transaction) fetchPendingBlockHeaderAndTxLoc(txHash *wire.ShaHash, offset uint32, length uint32) ([]byte, []byte, error) {
	// Nothing to do if the block is not pending to be written on commit.
	idx, exists := tx.pendingTransactions[*txHash]
	if !exists {
		return nil, nil, nil
	}
	...
	if endOffset < offset || endOffset > blockLen {
		str := fmt.Sprintf("transaction %s region offset %d, length %d "+
			"exceeds block length of %d", txHash,
			offset, length, blockLen)
		return nil, nil, makeDbErr(database.ErrBlockRegionInvalid, str, nil)
	}

	// Return the bytes from the pending block.
	return blockBytes[0:blockHdrSize:blockHdrSize], blockBytes[offset:endOffset:endOffset], nil
}

// FetchTransaction returns the raw serialized bytes for the given block region.
//
...
//
// This function is part of the database.Tx interface implementation.
func (tx *transaction) FetchTransaction(txHash *wire.ShaHash) ([]byte, []byte, error) {
	// Ensure transaction state is valid.
	if err := tx.checkClosed(); err != nil {
		return nil, nil, err
	}
	...
	txLoc, err := txBlockLocation(tx, txHash)
	if err != nil {
		return nil, nil, errNotExistTxLoc
	}
	if txLoc == nil {
		return nil, nil, nil // tx not found
	}
	...
	if tx.pendingBlocks != nil {
		blockHeader, regionBytes, err = tx.fetchPendingBlockHeaderAndTxLoc(txHash, txLoc.txOffset, txLoc.txLen)
		if err != nil {
			return nil, nil, err
		}
		if blockHeader != nil && regionBytes != nil {
			return blockHeader, regionBytes, nil
		}
	}

	// Read the region from the appropriate disk block file.
	blockHeader, regionBytes, err = tx.db.store.readBlockLocation(txLoc)
	if err != nil {
		return nil, nil, err
	}
...
// FetchTransactions returns the raw serialized bytes for the given block
// regions.
//
// For example, it is possible to directly extract Bitcoin transactions and/or
...
//
// This function is part of the database.Tx interface implementation.
func (tx *transaction) FetchTransactions(txHashs []wire.ShaHash) ([][]byte, [][]byte, error) {
	// Ensure transaction state is valid.
	if err := tx.checkClosed(); err != nil {
		return nil, nil, err
	}

	var txLocs []*transactionLocation

	// Look up the location of the transaction.
	for i := range txHashs {
		txLoc, err := txBlockLocation(tx, &txHashs[i])
		if err != nil {
			return nil, nil, errNotExistTxLoc
		}
		if txLoc == nil {
	...
	blockRegions := make([][]byte, len(txLocs))
	blockHeaders := make([][]byte, len(txLocs))
	fetchList := make([]bulkFetchData, 0, len(txLocs))
	for i := range txLocs {
		region := txLocs[i]
		...
		// bytes from there.
		if tx.pendingBlocks != nil {
			blockHeader, regionBytes, err := tx.fetchPendingBlockHeaderAndTxLoc(
				&txHashs[i], region.txOffset, region.txLen)
			if err != nil {
				return nil, nil, err
			}
			if blockHeader != nil && regionBytes != nil {
		...
		// keep locations that are not existing in the pending blocks
		fetchList = append(fetchList, bulkFetchData{region, i})
	}

	// To enhence read speed, sort by an occurence sequence of the region in a disk
	...
	for i := range fetchList {
		...
		region := txLocs[ri]
		blockHeader, regionBytes, err := tx.db.store.readBlockLocation(region)
		if err != nil {
			return nil, nil, err
		}
		blockRegions[ri] = regionBytes
		blockHeaders[ri] = blockHeader
	}

	return blockHeaders, blockRegions, nil
}

func (tx *transaction) updatePendingBlockTransaction(mtx *wire.MsgTx, offset uint32, length uint32) (bool, error) {
	txHash := mtx.TxSha()
	...
	if endOffset < offset || endOffset > blockLen {
		str := fmt.Sprintf("transaction %s region offset %d, length %d "+
			"exceeds block length of %d", txHash,
			offset, length, blockLen)
		return true, makeDbErr(database.ErrBlockRegionInvalid, str, nil)
	}
	...
	txBytes := w.Bytes()
	copy(blockBytes[offset:endOffset], txBytes)
	return true, nil
}
func (tx *transaction) UpdateTransaction(mtx *wire.MsgTx) (bool, error) {
	// Ensure transaction state is valid.
	if err := tx.checkClosed(); err != nil {
		return false, err
	}
	...
	txLoc, err := txBlockLocation(tx, &txHash)
	if err != nil {
		return false, errors.New("no entry in the block ID index")
	}
	if txLoc == nil {
	...
	// from there.
	if tx.pendingBlocks != nil {
		_, err = tx.updatePendingBlockTransaction(mtx, txLoc.txOffset, txLoc.txLen)
		if err != nil {
			return false, err
		}
	}
	...
	var bw bytes.Buffer
	mtx.Serialize(&bw)
	serializedData := bw.Bytes()

	// Update the region from the appropriate disk block file.
	err = tx.db.store.updateBlockLocation(txLoc, serializedData)
	if err != nil {
		return true, err
	}
	return true, nil
...
func (tx *transaction) close() {
	...
	tx.pendingTransactions = nil

	tx.pendingDisconnectBlock = nil
...
func (tx *transaction) writePendingAndCommit() error {
	...
	for _, blockData := range tx.pendingBlockData {
		...
		// convert and create a block from bytes
		block, err := btcutil.NewBlockFromBytes(blockData.bytes)
		if err != nil {
			rollback()
			return err
		}

		// add the block to tx index
		err = indexBlockTxs(tx, block, location)
		if err != nil {
			rollback()
			return err
		}
	...
	// disconnect block
	for _, disconnectBlock := range tx.pendingDisconnectBlock {
		log.Tracef("Disconnecting block %s", disconnectBlock.Sha().String())

		// remove tx infos from tx index
		disconnectBlockTxs(tx, disconnectBlock)

		// TODO block is disconnected, but still remains in a file.
		// It's actually not removed
	}
