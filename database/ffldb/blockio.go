...
var (
	updateSerializedChecksumEnabled = false
)
...
// TransactionLocation specifies a particular region of a block
...
func (s *blockStore) blockFileUpdatable(fileNum uint32) (*lockableFile, error) {
	uc := s.updateCursor
	uc.Lock()
	defer uc.Unlock()

	if uc.curFile.file != nil && fileNum != uc.curFileNum {
		_ = uc.curFile.file.Close()
		uc.curFile.file = nil
	}

	if uc.curFile.file == nil {
		fp, err := s.openWriteFileFunc(fileNum)
		if err != nil {
			return nil, err
		}
		uc.curFileNum = fileNum
		uc.curFile.file = fp
	}
...
func (s *blockStore) readBlock(loc blockLocation) ([]byte, error) {
	...
	if updateSerializedChecksumEnabled {
		...
		if serializedChecksum != calculatedChecksum {
			str := fmt.Sprintf("block data for block checksum "+
				"does not match - got %x, want %x",
	...
	if serializedNet != uint32(s.network) {
		str := fmt.Sprintf("block data for block is for the "+
			"wrong network - got %d, want %d", serializedNet,
			uint32(s.network))
...
// readBlockLocation reads the specified amount of data at the provided offset for
// a given block location. (+ blockHeader bytes)  The offset is relative to the
// start of the serialized block (as opposed to the beginning of the block record).
// This function automatically handles all file management such as opening and
...
func (s *blockStore) readBlockLocation(loc *transactionLocation) ([]byte, []byte, error) {
	// Get the referenced block file handle opening the file as needed.  The
	// function also handles closing files as needed to avoid going over the
	// max allowed open files.
	...
	txReadOffset := loc.blockOffset + 8 + loc.txOffset
	serializedTxData := make([]byte, loc.txLen)

	blockHeaderReadOffset := loc.blockOffset + 8
	serializedHeaderData := make([]byte, blockHdrSize)

	// Read block header and region
	_, err = blockFile.file.ReadAt(serializedHeaderData, int64(blockHeaderReadOffset))
	if err != nil {
		str := fmt.Sprintf("failed to read block header from block file %d, "+
			"offset %d, len %d: %v", loc.blockFileNum, blockHeaderReadOffset,
			loc.txLen, err)
		return nil, nil, makeDbErr(database.ErrDriverSpecific, str, err)
	}
	_, err = blockFile.file.ReadAt(serializedTxData, int64(txReadOffset))
	if err != nil {
		str := fmt.Sprintf("failed to read region from block file %d, "+
			"offset %d, len %d: %v", loc.blockFileNum, txReadOffset,
			loc.txLen, err)
		return nil, nil, makeDbErr(database.ErrDriverSpecific, str, err)
	}

	return serializedHeaderData, serializedTxData, nil
}

func (s *blockStore) updateBlockLocation(loc *transactionLocation, data []byte) error {

	// Get the referenced block file handle opening the file as needed.  The
	// function also handles closing files as needed to avoid going over the
	// max allowed open files.
	blockFile, err := s.blockFileUpdatable(loc.blockFileNum)
	if err != nil {
		return err
	}
	...
	// data for a block includes an initial 4 bytes for network + 4 bytes
	// for block length.  Thus, add 8 bytes to adjust.
	txUpdateOffset := loc.blockOffset + 8 + loc.txOffset

	// check data size to update
	if loc.txLen < uint32(len(data)) {
		str := fmt.Sprintf("failed to update region from block file %d, "+
			"offset %d, len %d, data %d: %v", loc.blockFileNum, txUpdateOffset,
			loc.txLen, len(data), err)
		return makeDbErr(database.ErrDriverSpecific, str, err)
	}

	serializedTxData := make([]byte, loc.txLen)
	copy(serializedTxData, data)

	// Write data
	_, err = blockFile.file.WriteAt(serializedTxData, int64(txUpdateOffset))
	blockFile.Unlock()
	if err != nil {
		str := fmt.Sprintf("failed to update region from block file %d, "+
			"offset %d, len %d, data %d: %v", loc.blockFileNum, txUpdateOffset,
