...
func testClosedTxInterface(tc *testContext, tx database.Tx) bool {
	...
	if k := bucket.Get(keyName); k != nil {
		tc.t.Error("Get on closed tx: did not return nil on closed tx")
		return false
	}
	...
	// bulk APIs below while looping.
	allBlockHashes := make([]wire.ShaHash, len(tc.blocks))
	allBlockRegions := make([]wire.ShaHash, len(tc.blocks))
	for i, block := range tc.blocks {
		blockHash := block.Sha()
		allBlockHashes[i] = *blockHash
		...
		txHash := block.Transactions()[0].Sha()
		allBlockRegions[i] = *txHash
		_, _, err = tx.FetchTransaction(txHash)
		if !checkDbError(tc.t, testName, err, wantErrCode) {
			return false
		}
	...
	_, _, err = tx.FetchTransactions(allBlockRegions)
	if !checkDbError(tc.t, testName, err, wantErrCode) {
		return false
	}
