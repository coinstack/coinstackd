...
func (cmd *blockRegionCmd) Execute(args []string) error {
	...
	return db.View(func(tx database.Tx) error {
		...
		regionBytes, err := tx.FetchBlock(blockHash)

		if err != nil {
			return err
		}

		result := make([]byte, regionLen)
		endOffset := startOffset + regionLen

		copy(result, regionBytes[startOffset:endOffset])

		log.Infof("Loaded block region in %v", time.Now().Sub(startTime))
		log.Infof("Double SHA256: %s", wire.DoubleSha256SH(result))
		log.Infof("Region Hex: %s", hex.EncodeToString(result))
		return nil
	})
}
