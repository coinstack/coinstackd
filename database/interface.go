...
type Bucket interface {
	...
	RangeCursor(prefix []byte) Cursor
...
type Tx interface {
	...
	// DisconnectBlock drop the provided block from the database
	DisconnectBlock(block *btcutil.Block) error
	...
	// FetchTransaction returns the block header and the raw serialized bytes
	// of the transaction for the given transaction hash.
	//
	// The interface contract guarantees at least the following errors will
	// be returned (other implementation-specific errors are possible):
	//   - ErrBlockNotFound if the requested block hash does not exist
	...
	// additional data copies and allows support for memory-mapped database
	// implementations.
	FetchTransaction(txHash *wire.ShaHash) ([]byte, []byte, error)

	// FetchTransactions returns the block headers and the raw serialized bytes
	// of transactions for the given transaction hashes.
	//
	// The interface contract guarantees at least the following errors will
	// be returned (other implementation-specific errors are possible):
	...
	// additional data copies and allows support for memory-mapped database
	// implementations.
	FetchTransactions(txHashs []wire.ShaHash) ([][]byte, [][]byte, error)

	UpdateTransaction(mtx *wire.MsgTx) (bool, error)
