...
func (u *pathUpGrader) upgrade(oldDbPath, netName string) {
	if u.err == nil {
		u.err = upgradeDBPathNet(oldDbPath, netName)
	}
}
...
func upgradeDBPaths() error {
	...
	u.upgrade(filepath.Join(oldDbRoot, "btcd.db"), "mainnet")
	u.upgrade(filepath.Join(oldDbRoot, "btcd_testnet.db"), "testnet")
	u.upgrade(filepath.Join(oldDbRoot, "btcd_regtest.db"), "regtest")
	if u.err != nil {
		return u.err
	}
