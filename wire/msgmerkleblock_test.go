...
func TestMerkleBlock(t *testing.T) {
	...
	data := make([]byte, 32)
	for i := 0; i < int(wire.MaxTxPerBlock()); i++ {
		rand.Read(data)
...
func TestMerkleBlockOverflowErrors(t *testing.T) {
	...
	// allowed tx hashes.
	var buf bytes.Buffer
	wire.TstWriteVarInt(&buf, pver, uint64(wire.MaxTxPerBlock()+1))
	numHashesOffset := 84
	...
	// allowed flag bytes.
	buf.Reset()
	wire.TstWriteVarInt(&buf, pver, uint64(wire.MaxFlagsPerMerkleBlock()+1))
