...
func (b *BlockChain) initChainState() error {
	...
	err := b.db.View(func(dbTx database.Tx) error {
		...
		// Load preceding nodes to memory to allow block reorganization
		blockHeight := int32(state.height) - 1
		workSum := node.workSum
		nextNode := b.bestNode
		for i := int32(0); i < minMemoryNodes()-2; i++ {
			header, err := dbFetchHeaderByHash(dbTx, prevHash)
			if nil != err {
				return err
			}
			node = newBlockNode(header, prevHash, blockHeight)
			node.inMainChain = true
			node.parent = nil
			node.children = append(node.children, nextNode)
			nextNode.parent = node
			node.workSum.Sub(workSum, node.workSum)
