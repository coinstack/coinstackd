...
func minMemoryNodes() int32 {
	// FIXME: since original BlocksPerRetarget() can be too small,
	// use original value
	return 2016 // time.Hour * 24 * 14 / time.Minute * 10
}
...
type BlockChain struct {
	...
	// Ignore the difficulty check upon accepting a block (For migration
	// between PoW and Interval Mining)
	noDifficultyCheck bool
...
func (b *BlockChain) addOrphanBlock(block *btcutil.Block) {
	...
	if len(b.orphans)+1 > b.maxOrphanBlocks {
		...
		log.Tracef("Evict the oldest orphan block %s due to the orphan blocks limit (%d)",
			b.oldestOrphan.block.Sha().String(), b.maxOrphanBlocks)
		b.oldestOrphan = nil
	}
...
func (b *BlockChain) disconnectBlock(node *blockNode, block *btcutil.Block, view *UtxoViewpoint) error {
	...
	err = b.db.View(func(dbTx database.Tx) error {
		var e error
		prevBlock, e = dbFetchBlockByHash(dbTx, prevNode.hash)
		return e
	})
	if err != nil {
		return err
	...
	err = b.db.Update(func(dbTx database.Tx) error {
		...
		e = dbRemoveBlockIndex(dbTx, block.Sha(), node.height)
		if e != nil {
			return e
		}
		...
		e = dbRemoveSpendJournalEntry(dbTx, block.Sha())
		if e != nil {
			return e
		}

		// Disconnect the block from the database
		e = dbTx.DisconnectBlock(block)
		if e != nil {
			return e
		}
		...
		if b.indexManager != nil {
			e = b.indexManager.DisconnectBlock(dbTx, block, view)
			if e != nil {
				return e
			}
		}
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List, flags BehaviorFlags) error {
	...
	for e := attachNodes.Front(); e != nil; e = e.Next() {
		...
		// Perform mining permission check
		if b.permissionManager != nil {
			err = b.CheckBlockPermission(b.permissionManager, block)
			if nil != err {
				return err
			}
		}
...
func (b *BlockChain) DisableExpiration(disable bool) {
	b.chainLock.Lock()
	b.noExpiration = disable
	b.chainLock.Unlock()
}
...
func (b *BlockChain) IsCurrent(timeSource MedianTimeSource) bool {
	...
	if b.bestNode.timestamp.Before(minus24Hours) && !b.noExpiration {
		return false
	}
...
type Config struct {
	...
	// Aceept a block without difficulty if true.
	...
	// PermissionManger	defines a permission manager to check mining permission
	//
	// This field can be nil if the caller does not wish to make use of a
	// permission manager.
	PermissionManager permission.Manager
}
...
func New(config *Config) (*BlockChain, error) {
	...
	b := BlockChain{
		...
		noDifficultyCheck:   config.IgnoreDifficulty,
	}
