...
const (
	defaultTargetTimespan           = time.Hour * 24 * 14
	defaultTargetSpacing            = time.Minute * 10
	defaultRetargetAdjustmentFactor = 4
	defaultSetupBlocks              = int32(100)
)
...
var (
	...
	TargetSpacing = defaultTargetSpacing

	// RetargetAdjustmentFactor is the adjustment factor used to limit
	// the minimum and maximum amount of adjustment that can occur between
	// difficulty retargets.
	RetargetAdjustmentFactor = defaultRetargetAdjustmentFactor

	// SetupBlocks indiciates the number of blocks to mine with minimum difficulty
	// for private network
	SetupBlocks = defaultSetupBlocks
...
// rate.
func BlocksPerRetarget() int32 {
	return int32(TargetTimespan / TargetSpacing)
}
...
// previous difficulty.
func minRetargetTimespan() int64 {
	return int64(TargetTimespan) / int64(RetargetAdjustmentFactor)
	// return int64(TargetTimespan / RetargetAdjustmentFactor)
}
...
// previous difficulty.
func maxRetargetTimespan() int64 {
	return int64(TargetTimespan) * int64(RetargetAdjustmentFactor)
}
...
func (b *BlockChain) calcEasiestDifficulty(bits uint32, duration time.Duration) uint32 {
	...
	adjustmentFactor := big.NewInt(int64(RetargetAdjustmentFactor))
...
func (b *BlockChain) findPrevTestNetDifficulty(startNode *blockNode) (uint32, error) {
	// Force the difficulty when cfg.PrivateNetMiningInterval > 0
	if TargetFixed == true {
...
func (b *BlockChain) calcNextRequiredDifficulty(lastNode *blockNode, newBlockTime time.Time) (uint32, error) {
	// Setup phase in priv network. Force the difficulty when
	// cfg.PrivateNetMiningInterval > 0.
	if b.chainParams.Net == wire.PrivNet &&
		(SetupBlocks >= lastNode.height+1 || TargetFixed == true) {
		return b.chainParams.PowLimitBits, nil
	}
