...
const (
	...
	// bitcoins (coinbase transactions) can be spent.
	//
	// *Warning*: Do not modify this number without a definite reason. If
	// you modify it for a test, revert it before commit!
	CoinbaseMaturity = 100

	errInvalidVersion = "new blocks with version %d are no longer valid"
)
...
func MaxSigOpsPerBlock() int {
	return int(wire.MaxBlockPayload / 50)
}
...
func CheckTransactionSanity(tx *btcutil.Tx) error {
	...
	if serializedTxSize > int(wire.MaxBlockPayload) {
...
func checkBlockSanity(block *btcutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
	...
	if numTx > int(wire.MaxBlockPayload) {
	...
	if serializedSize > int(wire.MaxBlockPayload) {
...
func (b *BlockChain) checkBlockHeaderContext(header *wire.BlockHeader, prevNode *blockNode, flags BehaviorFlags) error {
	...
	if !fastAdd {
		...
		// Ignore block difficulty check if b.noDifficultyCheck == true
		if !b.noDifficultyCheck && blockDifficulty != expectedDifficulty {
	...
	if !fastAdd {
		...
		if header.Version < 4 && b.isMajorityVersion(4, prevNode,
			...
			return ruleError(ErrBlockVersionTooOld, fmt.Sprintf(errInvalidVersion, header.Version))
		}
...
func (b *BlockChain) checkConnectBlock(node *blockNode, block *btcutil.Block, view *UtxoViewpoint, stxos *[]spentTxOut) error {
	...
	for i, tx := range transactions {
		...
		if enforceBIP0016 {
			...
			numP2SHSigOps, e := CountP2SHSigOps(tx, i == 0, view)
			if e != nil {
				return e
			}
	...
	var totalFees int64
	for _, tx := range transactions {
		txFee, e := CheckTransactionInputs(tx, node.height, view)
		if e != nil {
			return e
		}
