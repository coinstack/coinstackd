...
const (
	...
	// Size of a transaction entry.  It equals with a size of tx id
	txEntrySize = wire.HashSize
)
...
//
//   Field           Type            Size
//   tx hash         wire.ShaHash    32 bytes
//   -----
//   Total: 32 bytes per indexed tx
// -----------------------------------------------------------------------------
...
func deserializeAddrIndexEntry(serialized []byte, txID *wire.ShaHash) error {
	// Ensure there are enough bytes to decode.
	if len(serialized) < txEntrySize {
		return errDeserialize("unexpected end of data")
	}

	copy(txID[:], serialized[:len(txID)])
...
// this updates the address index to include the provided entry according to the
// level-based schema described in detail above.
func dbPutAddrIndexEntry(bucket internalBucket, addrKey [addrKeySize]byte, txid wire.ShaHash) error {
	...
	if len(level0Data)+len(txid) <= maxLevelBytes {
		var mergedData []byte
		if len(level0Data) > 0 {
			mergedData = make([]byte, len(level0Data)+len(txid))
			copy(mergedData, level0Data)
			copy(mergedData[len(level0Data):], txid[:])
		} else {
			mergedData = make([]byte, len(txid))
			copy(mergedData, txid[:])
		}
	...
	return bucket.Put(level0Key[:], txid[:])

}

// dbFetchAddrIndexEntries returns transaction ids referenced by
...
func dbFetchAddrIndexEntries(bucket internalBucket, addrKey [addrKeySize]byte, numToSkip, numRequested uint32, reverse bool) ([]wire.ShaHash, uint32, error) {
	...
	// number.
	results := make([]wire.ShaHash, numToLoad)
...
func (idx *AddrIndex) Init(bestHeight int32) error {
	// Nothing to do.
	return nil
}
...
type writeIndexData map[[addrKeySize]byte][]wire.ShaHash

// indexPkScript extracts all standard addresses from the passed public key
// script and maps each of them to the associated transaction using the passed
// map.
func (idx *AddrIndex) indexPkScript(data writeIndexData, pkScript []byte, txID *wire.ShaHash) {

	// This classify type of script class to extract address to index or check
	// special NullData that contains addr-data marker
	scriptClass, addrs, _, err := txscript.ExtractPkScriptAddrs(pkScript,
		idx.chainParams)
	...
	if scriptClass == txscript.NullDataTy {
		...
		addr, err := txscript.ParseAddrDataScript(pkScript, idx.chainParams)
		if err != nil {
			return
		}
		...
		if numTxns > 0 && indexedTxns[numTxns-1] == *txID {
			return
		}
		indexedTxns = append(indexedTxns, *txID)
	...
	for txIdx, tx := range block.Transactions() {
		...
		if txIdx != 0 {
			for _, txIn := range tx.MsgTx().TxIn {
				...
				idx.indexPkScript(data, pkScript, tx.Sha())
			}
		}

		for _, txOut := range tx.MsgTx().TxOut {
			idx.indexPkScript(data, txOut.PkScript, tx.Sha())
		}
	}
}
...
func (idx *AddrIndex) ConnectBlock(dbTx database.Tx, block *btcutil.Block, view *blockchain.UtxoViewpoint) error {
	...
	for addrKey, txIDs := range addrsToTxns {
		for _, txID := range txIDs {
			err := dbPutAddrIndexEntry(addrIdxBucket, addrKey, txID)
			if err != nil {
				return err
			}
		}
...
// TxIDsForAddress returns a slice of block IDs which identify each
...
func (idx *AddrIndex) TxIDsForAddress(dbTx database.Tx, addr btcutil.Address, numToSkip, numRequested uint32, reverse bool) ([]wire.ShaHash, uint32, error) {
	addrKey, err := addrToKey(addr)
	if err != nil {
		return nil, 0, err
	}

	var regions []wire.ShaHash
	var skipped uint32
	err = idx.db.View(func(dbTx database.Tx) error {
		var e error
		addrIdxBucket := dbTx.Metadata().Bucket(addrIndexKey)
		regions, skipped, e = dbFetchAddrIndexEntries(addrIdxBucket,
...
func (idx *AddrIndex) AddUnconfirmedTx(tx *btcutil.Tx, utxoView *blockchain.UtxoViewpoint) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		...
		// Extracts all addresses-txid pair in tx
		idx.indexPkScript(addrsToTxns, pkScript, tx.Sha())
	}

	// Index addresses of all created outputs.
	for _, txOut := range tx.MsgTx().TxOut {
		// Extracts all addresses-txid pair in tx
		idx.indexPkScript(addrsToTxns, txOut.PkScript, tx.Sha())
	}

	// for each addresses that tx contains
