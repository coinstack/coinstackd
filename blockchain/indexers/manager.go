...
func (m *Manager) Init(chain *blockchain.BlockChain) error {
	...
	err = m.db.Update(func(dbTx database.Tx) error {
		...
		meta := dbTx.Metadata()
		_, e := meta.CreateBucketIfNotExists(indexTipsBucketName)
		if e != nil {
			return e
		}
	...
	for _, indexer := range m.enabledIndexes {
		if err = indexer.Init(chain.BestSnapshot().Height); err != nil {
			return err
		}
		if chainRequired, ok := indexer.(ChainDepends); ok {
			chainRequired.SetChain(chain)
		}
	}
	...
	for i := len(m.enabledIndexes); i > 0; i-- {
		...
		for {
			...
			err = m.db.Update(func(dbTx database.Tx) error {
				...
				block, e := btcutil.NewBlockFromBytes(blockBytes)
				if e != nil {
					return e
				}
				...
				if indexNeedsInputs(indexer) {
					view, e = makeUtxoView(dbTx, block)
					if e != nil {
						return e
					}
				}
				...
				e = dbIndexDisconnectBlock(dbTx, indexer,
					block, view)
				if e != nil {
					return e
				}
...
func dbFetchTx(dbTx database.Tx, hash *wire.ShaHash) (*wire.MsgTx, error) {
	...
	_, txBytes, err := dbTx.FetchTransaction(hash)
	if err != nil {
		return nil, err
	}
	if txBytes == nil {
...
func dropIndex(db database.DB, idxKey []byte, idxName string) error {
	...
	err = db.Update(func(dbTx database.Tx) error {
		...
		var e error
		if e = indexesBucket.Delete(idxKey); e != nil {
			return e
		}

		if e = meta.DeleteBucket(idxKey); e != nil {
			return e
		}
