...
func (b *BlockChain) blockExists(hash *wire.ShaHash) (bool, error) {
	...
	// FIXME: try to repair index
	if exists {
		_, err = b.BlockByHash(hash)
		if err != nil {
			// block exists but index does not exist, so remove block
			// to give chance to reindexed
			return false, nil
		}
	}
...
func (b *BlockChain) ProcessBlock(block *btcutil.Block, timeSource MedianTimeSource, flags BehaviorFlags) (bool, error) {
	...
	if !prevHash.IsEqual(zeroHash) {
		prevHashExists, e := b.blockExists(prevHash)
		if e != nil {
			return false, e
		}
		if !prevHashExists {
