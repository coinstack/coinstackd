...
func (s *rpcServer) WebsocketHandler(conn *websocket.Conn, remoteAddr string,
	...
	// the connection.
	conn.SetReadDeadline(timeZeroVal) // nolint: errcheck
...
type BlockEvent func(*btcutil.Block)
type TxEvent func(*btcutil.Tx)
...
type wsNotificationManager struct {
	...
	// add-on for coinstack API notifications
	blockEvents map[interface{}]BlockEvent
	txEvents    map[interface{}]TxEvent
}
...
func queueHandler(in <-chan interface{}, out chan<- interface{}, quit <-chan struct{}) {
	...
	getNext := func(q *list.List) (headElem *list.Element, headVal interface{}) {
		headElem = q.Front()
		headVal = headElem.Value
		return
	}
...
func (m *wsNotificationManager) NotifyInv(address string, invVect *wire.InvVect) {
	n := &notificationInvRelay{
		address: address,
		invVect: invVect,
	}
...
type notificationInvRelay struct {
	address string
...
type notificationRegisterInv wsClient // TODO: add unregister
...
func (m *wsNotificationManager) notificationHandler() {
	...
	invNotifications := make(map[chan struct{}]*wsClient)
	...
	for {
		...
		case n, ok := <-m.notificationMsgs:
			...
			case *notificationTxAcceptedByMempool:
				...
				m.notifyTxEvents(n.tx)
			case *notificationInvRelay:
				m.notifyInvRelay(invNotifications, n.address, n.invVect)
			...
			case *notificationUnregisterNewMempoolTxs:
				wsc := (*wsClient)(n)
				delete(txNotifications, wsc.quit)
			case *notificationRegisterInv:
				wsc := (*wsClient)(n)
				invNotifications[wsc.quit] = wsc
...
// websocket client.
func (m *wsNotificationManager) RegisterInvUpdates(wsc *wsClient) {
	m.queueNotification <- (*notificationRegisterInv)(wsc)
}
...
func (*wsNotificationManager) notifyInvRelay(clients map[chan struct{}]*wsClient, address string, invVect *wire.InvVect) {

	// Notify interested websocket clients about the connected inv relay.
	invType := ""
	...
	ntfn := btcjson.NewInvRelayNtfn(address, invVect.Hash.String(), invType)
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
		rpcsLog.Error("Failed to marshal inv relay notification: "+
			"%v", err)
		return
	}
	for _, wsc := range clients {
...
func txHexString(tx *btcutil.Tx) string {
	...
	tx.MsgTx().Serialize(buf) // nolint: errcheck
...
func (m *wsNotificationManager) notifyBlockEvents(block *btcutil.Block) {
	// notify block events
	for _, event := range m.blockEvents {
		event(block)
	}
}

func (m *wsNotificationManager) AddBlockEvent(key interface{}, blockEvent BlockEvent) {
	m.blockEvents[key] = blockEvent
}

func (m *wsNotificationManager) RemoveBlockEvent(key interface{}) {
	delete(m.blockEvents, key)
}

func (m *wsNotificationManager) notifyTxEvents(tx *btcutil.Tx) {
	// notify tx events
	for _, event := range m.txEvents {
		event(tx)
	}
}

func (m *wsNotificationManager) AddTxEvent(key interface{}, txEvent TxEvent) {
	m.txEvents[key] = txEvent
}

func (m *wsNotificationManager) RemoveTxEvent(key interface{}) {
	delete(m.txEvents, key)
}
...
func newWsNotificationManager(server *rpcServer) *wsNotificationManager {
	return &wsNotificationManager{
		...
		blockEvents:       make(map[interface{}]BlockEvent),
		txEvents:          make(map[interface{}]TxEvent),
	}
}
...
func handleNotifyInv(wsc *wsClient, icmd interface{}) (interface{}, error) {
	wsc.server.ntfnMgr.RegisterInvUpdates(wsc)
	return nil, nil
}
