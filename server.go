...
type server struct {
	...
	announceInv          chan []*btcutil.Tx
	broadcast            chan broadcastMsg
	peerHeightsUpdate    chan updatePeerHeightsMsg
	wg                   sync.WaitGroup
	cutAllPeers          chan struct{}
	quit                 chan struct{}
	nat                  NAT
	db                   database.DB
	...
	utxoIndex *indexers.UtxoIndex
	permIndex *indexers.PermIndex
	oaIndex   *indexers.OAIndex
	ismIndex  *indexers.ISMIndex
	dtIndex   *indexers.DTIndex

	// Lock to protect mempool tx dump operation
	mempoolLock sync.RWMutex

	// enable debug api
	enableDebug bool

	// enable key-value store for a shinhan project
	enableKVStore bool
}
...
func (sp *serverPeer) addBanScore(persistent, transient uint32, reason string) {
	...
	if cfg.DisableBanning || (cfg.PrivateNet && cfg.PrivateNetBanning) {
		return
	}
...
func (sp *serverPeer) OnMemPool(p *peer.Peer, msg *wire.MsgMemPool) {
	...
	for i, txDesc := range txDescs {
		...
		if !sp.filter.IsLoaded() || sp.filter.MatchTxAndUpdate(txDesc.Tx) {
			...
			invMsg.AddInvVect(iv) // nolint: errcheck
...
func (sp *serverPeer) OnGetData(p *peer.Peer, msg *wire.MsgGetData) {
	...
	for _, iv := range msg.InvList {
		...
		switch iv.Type {
		case wire.InvTypeTx:
			err = sp.server.pushTxMsg(sp, &iv.Hash, nil, nil)
		case wire.InvTypeBlock:
			err = sp.server.pushBlockMsg(sp, &iv.Hash, nil, nil)
		case wire.InvTypeFilteredBlock:
			err = sp.server.pushMerkleBlockMsg(sp, &iv.Hash, nil, nil)
		default:
		...
		if err != nil {
			notFound.AddInvVect(iv) // nolint: errcheck
		}
	}
...
func (sp *serverPeer) OnGetHeaders(p *peer.Peer, msg *wire.MsgGetHeaders) {
	...
	if len(msg.BlockLocatorHashes) == 0 {
		...
		err = sp.server.db.View(func(dbTx database.Tx) error {
			var e error
			headerBytes, e = dbTx.FetchBlockHeader(&msg.HashStop)
			return e
		})
		if err != nil {
		...
		headersMsg.AddBlockHeader(&header) // nolint: errcheck
	...
	startIdx := int32(1)
	for _, hash := range msg.BlockLocatorHashes {
		height, e := chain.BlockHeightByHash(hash)
		if e == nil {
			// Start with the next hash since we know this one.
			startIdx = height + 1
			break
	...
	err = sp.server.db.View(func(dbTx database.Tx) error {
		for i := range hashList {
			headerBytes, e := dbTx.FetchBlockHeader(&hashList[i])
			if e != nil {
				return e
			}
...
// AnnounceNewTransactionsImmediately generates and relays inventory vectors
// immediately to the connected peers. This function should be called when new
// transactions are submitted through RPC through sendrawtransaction, emulating
// the behavior of bitcoin core
func (s *server) AnnounceNewTransactionsImmediately(newTxs []*btcutil.Tx) {
	s.announceInv <- newTxs
	// notify to RPC listeners
	for _, tx := range newTxs {
		if s.rpcServer != nil {
			// Notify websocket clients about mempool transactions.
			s.rpcServer.ntfnMgr.NotifyMempoolTx(tx, true)
...
func (s *server) handleAnnounceInvTxs(state *peerState, newTxs []*btcutil.Tx) {
	state.forAllPeers(func(sp *serverPeer) {
		if !sp.Connected() {
			return
		}
		invMsg := wire.NewMsgInvSizeHint(uint(len(newTxs)))
		for _, tx := range newTxs {
			// Generate the inventory vector and relay it.
			invVect := wire.NewInvVect(wire.InvTypeTx, tx.Sha())
			...
			invMsg.AddInvVect(invVect) // nolint: errcheck
		}
		// Queue the inventory to be sent to peer immediately
		sp.QueueMessage(invMsg, nil)

		for _, inv := range invMsg.InvList {
			// announce inventory queueing
			sp.server.rpcServer.ntfnMgr.NotifyInv(sp.Peer.Addr(), inv)
		}
	})
}
...
func (s *server) handleDonePeerMsg(state *peerState, sp *serverPeer) {
	...
	// If verack message is not received, the peer is not in any list. Even if
	// that is the case, try to reconnect in a private blockchain environment.
	if _, ok := list[sp.ID()]; ok || (cfg.PrivateNet && cfg.ForceReconnect) {
...
func (s *server) handleRelayInvMsg(state *peerState, msg relayMsg) {
	state.forAllPeers(func(sp *serverPeer) {
		...
		// have the inventory.
		queued := sp.QueueInventory(msg.invVect)

		// announce inventory queueing
		if queued {
			sp.server.rpcServer.ntfnMgr.NotifyInv(sp.Peer.Addr(), msg.invVect)
		}
	})
}
...
func newPeerConfig(sp *serverPeer) *peer.Config {
	...
	var nodePrivateKey *btcec.PrivateKey

	if cfg.PrivateNetNodeKey == "" {
		srvrLog.Warnf("Start server without privnetnodekey key")
	} else {
		// load key and sign ECDSA
		nodeKey, err := btcutil.DecodeWIF(cfg.PrivateNetNodeKey)
		nodePrivateKey = nodeKey.PrivKey
		if nil != err {
			srvrLog.Errorf("Fail to decode WIF privnetnodekey key: %s", err)
		}
	}
	...
	peerConfig := &peer.Config{
		...
		DB:                   sp.server.db,
		StallResponseTimeout: time.Duration(cfg.StallResponseTimeout) * time.Second,
	}
	if sp.server.permIndex != nil {
		peerConfig.PermissionManager = sp.server.permIndex
	}
	if nodePrivateKey != nil {
		peerConfig.NodePrivateKey = nodePrivateKey
	}
...
func (s *server) listenHandler(listener net.Listener) {
	AdmInfof(srvrLog, "Server listening on %s", listener.Addr())
...
func (s *server) retryConn(sp *serverPeer, initialAttempt bool) {
	...
	for {
		...
		case <-time.After(retryDuration):
			err := s.establishConn(sp)
			if err != nil {
				srvrLog.Warnf("Error while connecting to %s : %s", sp.Addr(), err.Error())
				retryDuration += connectionRetryInterval
...
out:
	for {
		...
		case invTxs := <-s.announceInv:
			s.handleAnnounceInvTxs(state, invTxs)
		...
		case <-s.cutAllPeers:
			// Disconnect all peers on watchdog expiration
			srvrLog.Debug("Get a disconnect-all message")
			state.forAllPeers(func(sp *serverPeer) {
				srvrLog.Tracef("Disconnect peer %s (watchdog expiration)", sp)
				sp.Disconnect()
			})
...
const (
	minRcTimeout = time.Minute
	maxRcTimeout = 5 * time.Minute
	maxInvSize   = wire.MaxInvPerMsg / 10
)

func getRebroadcastingTimeout() (timeout time.Duration) {
	if cfg.PrivateNetMiningInterval > 0 {
		timeout = 5 * time.Duration(cfg.PrivateNetMiningInterval) * time.Second
	} else if cfg.PrivateNetTargetSpacing > 0 {
		timeout = 5 * time.Duration(cfg.PrivateNetTargetSpacing) * time.Second
	}

	if timeout == 0 || timeout > maxRcTimeout {
		timeout = maxRcTimeout
	} else if timeout < minRcTimeout {
		timeout = minRcTimeout
	}

	return
}

func getForceRebroadcastingTimeout() time.Duration {
	return 10 * getRebroadcastingTimeout()
}

type pendingInvEntry struct {
	data      interface{}
	timeAdded time.Time
	timeSent  time.Time
}

func newTxs() []*btcutil.Tx {
	return make([]*btcutil.Tx, 0, maxInvSize)
}

func (s *server) maybeAnnouncePendingTx(txs []*btcutil.Tx, data interface{}) []*btcutil.Tx {
	txs = append(txs, data.(*btcutil.Tx))
	if len(txs) >= maxInvSize {
		peerLog.Debugf("Force rebroadcasting pending invs (len=%v)", len(txs))
		s.AnnounceNewTransactionsImmediately(txs)
		txs = newTxs()
	}
	return txs
}

func (s *server) flushPendingTx(txs []*btcutil.Tx) {
	// Flush the remaining txs.
	if cfg.ForceInvRebroadcasting &&
		len(txs) < maxInvSize && len(txs) > 0 {
		peerLog.Debugf("Force rebroadcasting pending invs (len=%v)", len(txs))
		s.AnnounceNewTransactionsImmediately(txs)
	}
}

func (s *server) releyPendingInvs(pendingInvs map[wire.InvVect]*pendingInvEntry) {
	// Make forceReboradcastingTimeout order of magnitude longer than the
	// normal rebroadcasting timeout
	forceRebroadcastingTimeout := getForceRebroadcastingTimeout()
	...
	for iv, entry := range pendingInvs {
		...
		// To make force rebroadcasting not too frequent
		if cfg.ForceInvRebroadcasting &&
			time.Now().Sub(entry.timeSent) > forceRebroadcastingTimeout {
			peerLog.Debugf(
				"Force relay Inv[%v, %v] - residence time %d seconds (last queued before %d seconds)",
				ivCopy.Type, ivCopy.Hash,
				time.Now().Sub(entry.timeAdded)/time.Second,
				time.Now().Sub(entry.timeSent)/time.Second)

			// Update time to prevent message flooding.
			entry.timeSent = time.Now()
			txs = s.maybeAnnouncePendingTx(txs, data)
		} else {
			s.RelayInventory(&ivCopy, data)
		}
	}

	s.flushPendingTx(txs)
}
...
func (s *server) rebroadcastHandler() {
	timer := time.NewTicker(getRebroadcastingTimeout())
	pendingInvs := make(map[wire.InvVect]*pendingInvEntry)
	...
	for {
		...
		case riv := <-s.modifyRebroadcastInv:
			...
			case broadcastInventoryAdd:
				now := time.Now()
				pendingInvs[*msg.invVect] = &pendingInvEntry{
					data:      msg.data,
					timeAdded: now,
					timeSent:  now,
				}
		...
		case <-timer.C:
			s.releyPendingInvs(pendingInvs)
...
func (s *server) Start() {
	...
	// Add saved mempool snapshot
	if cfg.DumpMempool {
		s.loadMempoolSnapshot()
	} else {
		srvrLog.Info("Option disabled to load mempool snapshot")
	}

	// Start watchdog if enabled
	if cfg.PrivateNet && (cfg.PrivateNetWatchdog || cfg.Watchdog) {
		// enable watchdog
		go watchSync(s, time.Second*time.Duration(cfg.WatchdogMessageDeadline), time.Second*time.Duration(cfg.WatchdogBlockDeadline))
	}
	// Start watchdog if enabled
	if !cfg.PrivateNet && (cfg.Watchdog || cfg.PrivateNetWatchdog) {
		// enable watchdog
		go watchSync(s, time.Second*time.Duration(cfg.WatchdogMessageDeadline), time.Second*time.Duration(cfg.WatchdogBlockDeadline))
	}

	go event.Handler(cfg.NodeID, cfg.EventTimeout)
}

func (s *server) StopServer(doMempoolDump bool) error {
	////////////////////////////////////////////////////////////////////////////
	// Utility anonymous functions
	////////////////////////////////////////////////////////////////////////////

	// Stop services (p2p & RPC listener, CPU miner).
	serviceStop := func() error {
		...
		if !cfg.DisableRPC {
			s.rpcServer.Stop() // nolint: errcheck
		}
		return nil
	}
	...
	dumpMempool := func() {
		if cfg.DumpMempool {
			if len(cfg.DumpMempoolAddresses) > 0 {
				s.DumpMempoolByAddresses(cfg.DumpMempoolAddresses)
			} else {
				s.DumpMempool()
			}
		} else {
			srvrLog.Info("Option disabled to dump mempool transactions")
		}
	}
...
func (s *server) Stop() error {
	return s.StopServer(true)
}

func (s *server) DumpMempool() {
	// Dump transactions in the mempool
	s.mempoolLock.Lock()
	defer s.mempoolLock.Unlock()

	nTx := s.txMemPool.MempoolTxDump(filepath.Join(cfg.DataDir, "mempoolsnapshot.data"))

	srvrLog.Infof("Saved %v mempool transactions", nTx)
}

func (s *server) DumpMempoolByAddresses(addressFilter []string) {
	// Dump transactions in the mempool
	s.mempoolLock.Lock()
	defer s.mempoolLock.Unlock()

	f, err := os.Create(filepath.Join(cfg.DataDir, "mempoolsnapshot.data"))
	if nil != err {
		srvrLog.Warnf("Failed to open mempool snapshot file")
		return
	}

	writer := csv.NewWriter(f)
	txProcessed := 0
	for _, address := range addressFilter {
		addr, _ := btcutil.DecodeAddress(address, s.chainParams)
		mempoolTxFetched := uint32(0)
		batchSize := 1024
		srvrLog.Warnf("Saving mempool txs for %v", address)
		for {
			fetchedBatch := 0
			mpTxns, _ := getMempoolTxnsForAddress(s, addr,
				mempoolTxFetched, uint32(batchSize))
			for _, tx := range mpTxns {
				mtxHex, err := messageToHex(tx.MsgTx())
				if nil != err {
					srvrLog.Warnf("Failed to process tx %v", tx.Sha().String())
				}
				// save mtxHex
				writer.Write([]string{mtxHex}) // nolint: errcheck
				txProcessed++
				mempoolTxFetched++
				fetchedBatch++
			}
	...
	writer.Flush()
	srvrLog.Infof("Saved %v mempool transactions", txProcessed)
...
func (s *server) rebroadcastMempool() {
	...
	for _, desc := range descs {
		iv := wire.NewInvVect(wire.InvTypeTx, desc.Tx.Sha())
		s.AddRebroadcastInventory(iv, desc.Tx)
	}

	if len(descs) > 0 {
		srvrLog.Infof(
			"From the mempool snapshot file %v transactions added to the rebroadcast list",
			len(descs))
	}
}

var memsnapWg sync.WaitGroup

func feedTx(f *os.File, txChannel chan<- string) {
	...
	// Wait until all the go-routines exit. This ensure that all the fed
	// transactions are processed.
	memsnapWg.Wait()
}

func (s *server) loadTx(tid int, txchannel <-chan string) {
	defer memsnapWg.Done()

	//pull string data from channel
	for {
		rawTx, ok := <-txchannel

		if ok == false {
			// kill worker
			srvrLog.Debug("Terminate load mempool worker #", tid)
			return
		}
		...
		if err != nil {
			srvrLog.Infof("Skipping tx: %v", err.Error())
			continue
		}
		msgtx := wire.NewMsgTx()
		...
		if nil != err {
			srvrLog.Warnf("tx rejected - %v: %s", msgtx.TxSha(), err.Error())
			continue
		}
		srvrLog.Debugf("%d: Loaded tx - %v", tid, msgtx.TxSha())
	}
}

func (s *server) runWorkers() chan<- string {
	// make tx string channel
	txChannel := make(chan string)
	nWorkers := runtime.NumCPU() * 2

	memsnapWg.Add(nWorkers)

	// run load tx workers
	for i := 0; i < nWorkers; i++ {
		srvrLog.Debug("Run load mempool worker #", i)
		go s.loadTx(i, txChannel)
	}

	return txChannel
}

func (s *server) loadMempoolSnapshot() {
	f, err := os.Open(filepath.Join(cfg.DataDir, "mempoolsnapshot.data"))
	if nil != err {
		srvrLog.Warnf("Failed to open mempool snapshot file")
		return
	}

	txChannel := s.runWorkers()

	// The function below feed transactions from a mempool snapshot to
	// worker go-routines. It wait until all those transactions are
	// processed. Such an wait is necessary since some transactions may not
	// be added to the rebroadcast list without it and, in turn, they may
	// remain forever in the mempool if the corresponding node do not
	// generate blocks.
	feedTx(f, txChannel)

	// After all the tranactions are processed, add them to the rebroadcast
	// list.
	s.rebroadcastMempool()

	srvrLog.Info("Successfully Finish to Load Mempool Data")
}
...
func newServer(
	...
	if !cfg.DisableListen {
		...
		if len(cfg.ExternalIPs) != 0 {
			...
			for _, sip := range cfg.ExternalIPs {
				...
				na, e := amgr.HostToNetAddress(host, eport,
					services)
				if e != nil {
				...
				if e != nil {
					amgrLog.Warnf("Skipping specified external IP: %v", e)
				}
			}
	...
	nowc:
		...
		if cfg.EnableP2PTLS {
			// No automatic generation. TLS cert and key file is required.
			if !fileExists(cfg.P2PKey) || !fileExists(cfg.P2PCert) {
				return nil, errors.New("PEER: P2P on TLS is enabled, but missing certificate or key file for p2p communication")
			}
	...
	s := server{
		...
		announceInv:          make(chan []*btcutil.Tx),
		broadcast:            make(chan broadcastMsg, cfg.MaxPeers),
		cutAllPeers:          make(chan struct{}),
		quit:                 make(chan struct{}),
		...
		enableDebug:          cfg.EnableDebugAPI,
		enableKVStore:        cfg.EnableKeyValueStore,
	}
	...
	s.utxoIndex = indexers.NewUtxoIndex(chainParams)
	indexes = append(indexes, s.utxoIndex)

	if cfg.OAIndex {
		s.oaIndex = indexers.NewOAIndex(chainParams, db)
		indexes = append(indexes, s.oaIndex)
	}

	if cfg.NoISMIndex {
		s.ismIndex = nil
	} else {
		ismIndex := indexers.NewISMIndex(chainParams, db, cfg.Ephemeral)
		s.ismIndex = ismIndex
		indexes = append(indexes, ismIndex)
		if cfg.PrivateNet {
			if cfg.PrivateNetNodeGroupKey != "" {
				s.ismIndex.SetNodeGroupKey(cfg.PrivateNetNodeGroupKey)
			}
			s.ismIndex.SetNodeID(cfg.NodeID)
			if cfg.PrivateNetNodeKey != "" && cfg.EncryptStateDB {
				s.ismIndex.SetInstanceEncryptKey(cfg.PrivateNetNodeKey)
			}
			if cfg.DisableLStateCache {
				s.ismIndex.DisableLStateCache()
			}
			if cfg.Ephemeral {
				s.ismIndex.EnableEphemeral()
			}
			if cfg.PurgeOldUndoStage {
				s.ismIndex.EnablePurgeOldUndoStage()
			}
		}
	}

	if cfg.PrivateNet && cfg.DtIndex {
		// add DeleteTx index
		indxLog.Info("DTIndex is enabled")
		s.dtIndex = indexers.NewDTIndex(chainParams, db, s.addrIndex)
		indexes = append(indexes, s.dtIndex)
	}

	if cfg.PrivateNet && cfg.PrivateNetNodeKey != "" {
		indxLog.Info("Permission management enabled")
		s.permIndex = indexers.NewPermIndex(chainParams)
		indexes = append(indexes, s.permIndex)
	}
	...
	bm, err := newBlockManager(&s, indexManager, cfg.MaxOrphanBlocks, cfg.FastAdd)
	if err != nil {
		return nil, err
	}
	s.blockManager = bm
	...
	txC := mempoolConfig{
		Policy: mempoolPolicy{
			...
			MaintainOrderList:    cfg.BlockGenSeqMode,
		},
	...
	if s.permIndex != nil {
		txC.PermissionManager = s.permIndex
	}
	if s.oaIndex == nil {
		txC.OAIndex = &indexers.NoOpMempoolOAIndex{}
	}
	...
	policy := mining.Policy{
		...
		BlockGenSeqMode:   cfg.BlockGenSeqMode,
	}
	s.cpuMiner = newCPUMiner(&policy, &s)
	s.cpuMiner.SetNumWorkers(cfg.NumMiners)
	...
	if cfg.DisableExpiration {
		s.blockManager.chain.DisableExpiration(true)
	}
