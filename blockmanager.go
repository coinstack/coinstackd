...
func (b *blockManager) startSync(peers *list.List) {
	...
	for e := peers.Front(); e != nil; e = enext {
		...
		//
		// Add the second condition to select the best among the
		// current peers.
		if sp.LastBlock() < best.Height ||
			(betterPeer != nil && sp.LastBlock() <= betterPeer.LastBlock()) {
			peers.Remove(e)
			continue
		}
	...
	if betterPeer != nil {
		bestPeer = betterPeer
		bmgrLog.Infof("Selected Sync Peer: address %v, height %d (current node's heigh %d)",
	...
	if bestPeer != nil {
		...
		if b.nextCheckpoint != nil &&
			...
			bestPeer.PushGetHeadersMsg(locator, b.nextCheckpoint.Hash) // nolint: errcheck
			b.headersFirstMode = true
			bmgrLog.Infof("Downloading headers for blocks %d to "+
				"%d from peer %s", best.Height+1,
				b.nextCheckpoint.Height, bestPeer.Addr())
		} else {
			bestPeer.PushGetBlocksMsg(locator, &zeroHash) // nolint: errcheck
		}
		b.syncPeer = bestPeer
	} else {
...
func (b *blockManager) handleTxMsg(tmsg *txMsg) {
	...
	if err != nil {
		...
		if _, ok := err.(RuleError); ok {
			bmgrLog.Warnf("Rejected transaction %v from %s: %v",
...
func (b *blockManager) handleBlockMsg(bmsg *blockMsg) {
	...
	if isOrphan {
		...
		if err != nil {
			...
			bmsg.peer.PushGetBlocksMsg(locator, orphanRoot) // nolint: errcheck
		}
	} else {
	...
	if blkShaUpdate != nil && heightUpdate != 0 {
		...
		if isOrphan || b.current() {
			go b.server.UpdatePeerHeights(blkShaUpdate, heightUpdate, bmsg.peer)
		}
	}
...
func (b *blockManager) fetchHeaderBlocks() {
	...
	for e := b.startHeader; e != nil; e = e.Next() {
		...
		if !haveInv {
			...
			gdmsg.AddInvVect(iv) // nolint: errcheck
			numRequested++
		}
	...
	for i, iv := range invVects {
		...
		if iv.Type == wire.InvTypeBlock {
			...
			if b.chain.IsKnownOrphan(&iv.Hash) {
				...
				imsg.peer.PushGetBlocksMsg(locator, orphanRoot) // nolint: errcheck
				continue
			}
			...
			if i == lastBlock {
				...
				imsg.peer.PushGetBlocksMsg(locator, &zeroHash) // nolint: errcheck
			}
		}
	}
...
// handleGetDebugInfoMsg returns hash strings of internal map for debuging
func (b *blockManager) handleGetDebugInfoMsg() map[string][]string {
	ret := make(map[string][]string)

	for hash := range b.rejectedTxns {
		ret["rejectedTxns"] = append(ret["rejectedTxns"], hash.String())
	}

	for hash := range b.requestedTxns {
		ret["requestedTxns"] = append(ret["requestedTxns"], hash.String())
	}

	for hash := range b.requestedBlocks {
		ret["requestedBlocks"] = append(ret["requestedBlocks"], hash.String())
	}
...
out:
	for {
		...
		case m := <-b.msgChan:
			...
			case getDebugInfoMsg:
				msg.reply <- b.handleGetDebugInfoMsg()
...
type descSorter []*mempoolTxDesc

func (a descSorter) Len() int           { return len(a) }
func (a descSorter) Swap(i, j int)      { a[i], a[j] = a[j], a[i] }
func (a descSorter) Less(i, j int) bool { return a[i].Added.Before(a[j].Added) }
...
func (b *blockManager) handleNotifyMsg(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockConnected:
		...
		// process unconfirmed txs to ismindex again
		if b.server.ismIndex != nil && b.server.ismIndex.EphemeralEnabled {
			best := b.chain.BestSnapshot()
			descs := b.server.txMemPool.TxDescs()
			sort.Sort(descSorter(descs)) // sort according to entry order to mempool
			for _, desc := range descs {
				utxoViewPoint, err := b.chain.FetchUtxoView(desc.Tx)
				if nil != err {
					continue
				}
				b.server.ismIndex.AddUnconfirmedTx(desc.Tx, utxoViewPoint, best)
			}
			// wait until all ephemeral transactions are processed as wel
			b.server.ismIndex.EphemeralLock.Unlock()
		}
	...
	case blockchain.NTBlockDisconnected:
		...
		for _, tx := range block.Transactions()[1:] {
			...
			if err != nil {
				...
				bmgrLog.Warnf("Remove tx(%s) during disconnect block caused by: %s",
					tx.Sha().String(), err.Error())
			}
		}
...
func (b *blockManager) Stop() error {
	...
	AdmInfof(bmgrLog, "Block manager shutting down")
	close(b.quit)
	b.wg.Wait()
	return nil
}
...
// GetDebugInfo returns the current requested txs, requested blocks, and rejected txs.
func (b *blockManager) GetDebugInfo() map[string][]string {
	reply := make(chan map[string][]string)
	b.msgChan <- getDebugInfoMsg{reply: reply}
	return <-reply
}
...
func newBlockManager(s *server, indexManager blockchain.IndexManager, maxOrphanBlocks int, fastAdd bool) (*blockManager, error) {
	msgChanSize := func() int {
		defaultSize := cfg.MaxPeers * 3
		if cfg.BmChanSize > defaultSize {
			return cfg.BmChanSize
		}
		return defaultSize
	}
	...
	config := &blockchain.Config{
		...
		IgnoreDifficulty: cfg.IgnoreDifficulty,
	}
	if s.permIndex != nil {
		config.PermissionManager = s.permIndex
	}

	var err error
	bm.chain, err = blockchain.New(config)
	if err != nil {
		return nil, err
	}
...
func loadBlockDB() (database.DB, error) {
	...
	removeRegressionDB(dbPath) // nolint: errcheck
