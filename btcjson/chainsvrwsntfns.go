...
const (
	...
	// InvRelayNtfnMethod is the method used for notifications from
	// the chain server that a inventory has been processed
	InvRelayNtfnMethod = "invrelay"
)
...
type InvRelayNtfn struct {
	Address string
	Hash    string
	InvType string
}

func NewInvRelayNtfn(address string, hash string, invType string) *InvRelayNtfn {
	return &InvRelayNtfn{
		Address: address,
		Hash:    hash,
		InvType: invType,
...
func init() {
	...
	MustRegisterCmd(InvRelayNtfnMethod, (*InvRelayNtfn)(nil), flags)
}
