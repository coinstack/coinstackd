...
// CheckPermissionCmd defines the verifymessage JSON-RPC command.
type CheckPermissionCmd struct {
	Address    string
	Permission string
}

// NewCheckPermissionCmd returns a new instance which can be used to issue a
// checkpermission JSON-RPC command.
func NewCheckPermissionCmd(address, permission string) *CheckPermissionCmd {
	return &CheckPermissionCmd{
		Address:    address,
...
func init() {
	...
	MustRegisterCmd("checkpermission", (*CheckPermissionCmd)(nil), flags)
}
