...
func TestChainSvrCmds(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			newCmd: func() (interface{}, error) {
				return btcjson.NewCmd("checkpermission", "1Address", "invoke")
			},
			staticCmd: func() interface{} {
				return btcjson.NewCheckPermissionCmd("1Address", "invoke")
			},
			marshalled: `{"jsonrpc":"1.0","method":"checkpermission","params":["1Address","invoke"],"id":1}`,
			unmarshalled: &btcjson.CheckPermissionCmd{
				Address:    "1Address",
				Permission: "invoke",
			},
		},
