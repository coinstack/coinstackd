...
func NewBlockTemplate(policy *mining.Policy, server *server, payToAddress btcutil.Address) (*BlockTemplate, error) {
	...
	var sourceTxns []*mining.TxDesc
	if policy.BlockGenSeqMode == false {
		sourceTxns = txSource.MiningDescs() //legacy mode
	} else {
		sourceTxns = txSource.MiningDescsSeqMode(policy.BlockMaxSize)
	}
...
mempoolLoop:
	for _, txDesc := range sourceTxns {
		...
		if cfg.MinerFastTxAdd == true && cutOffSize > policy.BlockMaxSize {
			break
		}
	}
	...
	for priorityQueue.Len() > 0 {
		...
		if blockSigOps+numSigOps < blockSigOps ||
			blockSigOps+numSigOps > int64(blockchain.MaxSigOpsPerBlock()) {
		...
		if server.permIndex != nil {
			// check writer permission
			err = server.blockManager.chain.CheckWriterPermission(tx, blockUtxos)

			if err != nil {
				minrLog.Tracef("Skipping tx %s due to error in "+
					"CheckWriterPermission: %v", tx.Sha(), err)
				logSkippedDeps(tx, deps)
				continue
			}
		}
		...
		spendTransaction(blockUtxos, tx, nextBlockHeight) // nolint: errcheck
