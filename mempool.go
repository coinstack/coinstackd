...
type mempoolConfig struct {
	...
	OAIndex indexers.MempoolOAIndexer
...
type mempoolPolicy struct {
	...
	// MaintainOrderlist defines whether to maintain list of transaction
	// in accepted-order or not.
	MaintainOrderList bool
}
...
func (mp *txMemPool) limitNumOrphans() error {
	if len(mp.orphans)+1 > mp.cfg.Policy.MaxOrphanTxs &&
		...
		txmpLog.Debugf(
			"Evict transaction %v from orphans pool due to the limit",
			foundHash)
...
func (mp *txMemPool) addOrphan(tx *btcutil.Tx) {
	...
	mp.limitNumOrphans() // nolint: errcheck
...
func (mp *txMemPool) removeTransaction(tx *btcutil.Tx, removeRedeemers bool) {
	...
	if txDesc, exists := mp.pool[*txHash]; exists {
		...
		mp.cfg.UtxoIndex.RemoveUnconfirmedTx(tx)
		mp.cfg.OAIndex.RemoveUnconfirmedTx(txHash)
		...
		if mp.cfg.Policy.MaintainOrderList {
			mp.orderList.Remove(txDesc.self)
			txDesc.self = nil
		}
...
func (mp *txMemPool) addTransaction(utxoView *blockchain.UtxoViewpoint, tx *btcutil.Tx, height int32, fee int64) {
	...
	mp.cfg.UtxoIndex.AddUnconfirmedTx(tx, utxoView, func(txHash *wire.ShaHash) (*btcutil.Tx, bool) {
		if txDesc, exists := mp.pool[*txHash]; exists {
	...
	mp.cfg.OAIndex.AddUnconfirmedTx(tx, utxoView)

	if mp.cfg.ISMIndex != nil && mp.cfg.ISMIndex.EphemeralEnabled {
		best := mp.cfg.Chain.BestSnapshot()
		mp.cfg.ISMIndex.AddUnconfirmedTx(tx, utxoView, best)
	}
	if mp.cfg.Policy.MaintainOrderList {
		txDesc.self = mp.orderList.PushBack(txDesc)
	}
}
...
func (mp *txMemPool) maybeAcceptTransaction(tx *btcutil.Tx, isNew, rateLimit bool) ([]*wire.ShaHash, error) {
	...
	// check permission index when checking permission is enabled
	if mp.cfg.PermissionManager != nil {
		err = mp.cfg.Chain.CheckWriterPermission(tx, utxoView)
...
// Returns the number of normal and orphan transactions and the total size in bytes.
func (mp *txMemPool) GetMempoolStatus() map[string]int {
	mp.RLock()
	defer mp.RUnlock()
	...
	mplen := len(mp.pool)
	result["mempool"] = mplen

	orphanlen := len(mp.orphans)
	result["orphanpool"] = orphanlen

	return result
}

// Returns orphan transaction hashes
func (mp *txMemPool) GetOrphanTxList() []string {
	mp.RLock()
	defer mp.RUnlock()
	txList := make([]string, 0, len(mp.orphans))
	for hash := range mp.orphans {
		txList = append(txList, hash.String())
	}
	return txList
}

// Returns transaction hashes in mempool
func (mp *txMemPool) GetMempoolTxList() []string {
	mp.RLock()
	defer mp.RUnlock()
	txList := make([]string, 0, len(mp.pool))
...
func (mp *txMemPool) MempoolTxDump(filePath string) int {
	f, err := os.Create(filePath)
	if nil != err {
		txmpLog.Warnf("Failed to open %v", filePath)
		return 0
	}
	...
	for _, desc := range descs {
		mtxHex, err := messageToHex(desc.Tx.MsgTx())
		if nil != err {
			txmpLog.Warnf("Failed to process tx %v", desc.Tx.Sha().String())
			continue
		}
	...
	if err = writer.Error(); err != nil {
		txmpLog.Warnf("Failed to flush mempool data: %v", err.Error())
	}
	f.Close()
...
func (mp *txMemPool) OrphanTxDump(filePath string) int {
	f, err := os.Create(filePath)
	if nil != err {
		txmpLog.Warnf("Failed to open %v", filePath)
		return 0
	...
	for _, tx := range txs {
		...
		if nil != err {
			txmpLog.Warnf("Failed to process tx %v", tx.Sha().String())
			continue
		}
		writer.Write([]string{mtxHex}) // nolint: errcheck
	}
...
func (mp *txMemPool) DumpAllMemTx() map[string]int {
	size := make(map[string]int)

	l := mp.MempoolTxDump(filepath.Join(cfg.DataDir, "mempool.data"))
	size["mempool"] = l

	l = mp.OrphanTxDump(filepath.Join(cfg.DataDir, "orphans.data"))
	size["orphans"] = l
...
func (mp *txMemPool) OrphanTxs() []*btcutil.Tx {
	mp.RLock()
	defer mp.RUnlock()

	txs := make([]*btcutil.Tx, len(mp.orphans))
	i := 0
...
func (mp *txMemPool) MiningDescsSeqMode(blockMaxSize uint32) []*mining.TxDesc {
	mp.RLock()
	defer mp.RUnlock()

	length := mp.orderList.Len()
	txSize := uint32(0)
	descs := make([]*mining.TxDesc, 0, length)
	for item := mp.orderList.Front(); item != nil; item = item.Next() {
		desc := item.Value.(*mempoolTxDesc)
		txSize += uint32(desc.TxDesc.Tx.MsgTx().SerializeSize())
		if blockMaxSize < txSize {
			break
		}
